...
/*old----------------*/
/*------- config the printf() function tx the data to rs232 serial port start---------*/
#ifdef __GNUC__
...
uint32_t spiRxFlag = 0;
uint32_t spiTxFlag = 0;
uint32_t rtuRxCount = 0;
uint32_t rtuTxCount = 0;
uint32_t rtuErrCount = 0;

#define RTU_ERR_MAX 256
uint32_t peTime[RTU_ERR_MAX] = {0};
uint32_t feTime[RTU_ERR_MAX] = {0};
uint32_t neTime[RTU_ERR_MAX] = {0};
uint32_t oreTime[RTU_ERR_MAX] = {0};
uint16_t peTimCount = 0;
uint16_t feTimCount = 0;
uint16_t neTimCount = 0;
uint16_t oreTimCount = 0;
...
//		  HAL_Delay(1);
////		  HAL_UART_Transmit(&huart2, (uint8_t *)"RTUTest\r\n", 9, 10);
...
//
			...
			/*IO Extender--------------------*/
//		  gVar.mbSerial.txCount = gVar.mbSerial.txCount + 1;
//		  if(gVar.mbSerial.txCount%3)
//		  gVar.mbSerial.rxCount = gVar.mbSerial.rxCount+ 1;
////			DBG_SPRINT_NL("rxCount: %ld, txCount: %ld", gVar.mbSerial.rxCount, gVar.mbSerial.txCount);
////			gVar.led[LED_1].state = 0;
...
static void MX_IWDG1_Init(void)
{
  ...
  hiwdg1.Init.Window = 3125;
  hiwdg1.Init.Reload = 3125;
